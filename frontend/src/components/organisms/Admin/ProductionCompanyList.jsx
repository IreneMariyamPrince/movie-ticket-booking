...
import productionCompaniesApi from '../../services/ProductionCompaniesApi';
...
const ProductionCompanyList = () => {
    ...
    const fetchProductionCompanies = async () => {
        setLoading(true);
        try {
            const data = await productionCompaniesApi.productionCompaniesList(
                page + 1,
                rowsPerPage,
                searchQuery,
                filterDate,
            ...
            setProductionCompanies(data.users);
    ...
    useEffect(() => {
        fetchProductionCompanies(); // Fetch production companies when dependencies change
    }, [page, rowsPerPage, searchQuery, filterDate, selectedStatus]); // Add selectedStatus to dependencies
                    ...
                    <div className='card my-4 mb-0'>
                        <div className='card-header p-0 position-relative mt-n4 mx-3 z-index-2'>
                            <div className='bg-gradient-primary shadow-primary border-radius-lg pt-4 px-3 pb-3 d-flex align-items-center justify-content-between'>
                                <h6 className='text-white text-capitalize ps-3 mb-0'>Production Companies List</h6>
                                    ...
                                    <table className='table align-items-center mb-0'>
                                        ...
                                        <tbody>
                                            {productionCompanies.map((productionCompany, index) => {
                                                const { FirstName, LastName, Email, UserStatus, PhoneNumber, ProfilePictureUrl } = productionCompany;
